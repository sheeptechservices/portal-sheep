...
async function rodar(P) {
  ...
  await P.salvarMatrizPapel(db, 'membro', [
    'solicitacoes:ver', 'solicitacoes:comentar', 'solicitacoes:mover',
  ...
  eq('DEPS pago NÃO', P.pode(perm, ['solicitacoes:deps', 'credito:deps']), false);
  ...
  eq('liberado devolve null', await P.exigir(db, membro, 'solicitacoes:ver'), null);
  ...
  const m = await P.salvarMatrizPapel(db, 'membro', ['solicitacoes:ver', 'inventada:tudo', 'usuarios:gerenciar'], admin);
  eq('só a válida sobrou', m.chaves.join(','), 'solicitacoes:ver');
